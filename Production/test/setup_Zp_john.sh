...
if [[ -n "$BATCH" ]]; then
	...
	git config --global --add user.name "Grace Cummings"
	git config --global --add user.email "graceelainecummings@gmail.com"
	git config --global --add user.github "gracecummings"
fi
if [[ "${DEBUG}" == "true" ]]; then
...
git clone ${ACCESS_GITHUB}${FORK}/ZpAnomalon_TreeMaker.git TreeMaker -b ${BRANCH}
